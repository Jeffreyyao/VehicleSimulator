...
class Node:
    def __init__(self, state, parent=None, t_from_parent=None, path_from_parent=None):
        self.state = state
...
class DynRRTPlanner:
    ...
    def get_action_greedy(self, state_start, point_end):
        min_dist = (self.size[0]**2+self.size[1]**2)**0.5
        min_action = None
        for u_steer in np.linspace(-self.max_steer,self.max_steer,5):
            action = [u_steer,self.max_velocity]
            state_new = self.rk_solver.RK4(state_start,action,self.tau)
            dist = np.linalg.norm(np.array(state_new[:2])-np.array(point_end))
            if dist<min_dist:
                min_dist = dist
                min_action = action
        return min_action

    def steer(self, state_start, point_end, t):
        path = []
        state_start = list(state_start) # RK4 must take in list objects
        for _ in np.arange(0,t,self.tau):
            path.append(state_start.copy())
            action = self.get_action_greedy(state_start,point_end)
            state_start = self.rk_solver.RK4(state_start,action,self.tau)
            if not self.point_is_valid(state_start[:2]): return None
        return path
    ...
    def rrt(self):
        for i in range(self.k):
            ...
            path = self.steer(node_nearest.state, point_rand, t_rand)
            if path != None:
                state_new = path[-1]
